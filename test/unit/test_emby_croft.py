...
from emby_client import requests
...
will actually call the emby server and handle real reasponses. There is probably
...
class TestEmbyCroft(object):
    ...
    def test_auth_mock(self):
        with mock.patch('requests.post') as MockRequestsPost:
            ...
            assert emby_croft.client.auth.token is TestEmbyCroft.auth_server_response["AccessToken"]
            assert emby_croft.client.auth.user_id is TestEmbyCroft.auth_server_response["User"]["Id"]
